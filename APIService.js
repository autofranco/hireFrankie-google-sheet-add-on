...
const APIService = {
  ...
  testNetworkConnection() {
    try {
      ...
      if (response.getResponseCode() === 200) {
        SpreadsheetApp.getUi().alert('✅ 网络连接正常！\n可以存取外部网站。');
      } else {
        SpreadsheetApp.getUi().alert('❌ 网络连接异常\n状态码: ' + response.getResponseCode());
      }
      
    } catch (error) {
      console.error('网络连接测试失败:', error);
      SpreadsheetApp.getUi().alert('❌ 网络连接测试失败：\n' + error.message);
    }
  },
  ...
  testAPIConnection() {
    try {
      ...
      SpreadsheetApp.getUi().alert('API 连接测试成功！\n\n回应内容：\n' + 
        result.substring(0, 200) + (result.length > 200 ? '...' : ''));
      
    } catch (error) {
      console.error('API 测试失败:', error);
      SpreadsheetApp.getUi().alert('API 连接测试失败：\n' + error.message);
    }
  },
  ...
  callPerplexityAPI(prompt) {
    ...
    if (!PERPLEXITY_API_KEY) {
      throw new Error('请先设定 Perplexity API Key');
    }
    ...
    if (responseCode !== 200) {
      ...
      throw new Error(`Perplexity API 错误: ${errorMessage}`);
    }
    ...
    try {
      ...
      throw new Error('API 回应格式错误: ' + responseText);
    }
    
    if (!responseData.choices || !responseData.choices[0] || !responseData.choices[0].message) {
      throw new Error('API 回应格式异常: ' + responseText);
    }
